...
import { PRODUCTS_DATA, PRODUCTS_PAGE_CONTENT } from "@/constants/products";
...
export default function ProductsPage() {
  ...
  const pageContent = PRODUCTS_PAGE_CONTENT[language];
      ...
      <div className="bg-white py-12 border-b border-slate-100">
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8">
            <h1 className="text-4xl font-bold">{pageContent.h1}</h1>
            ...
            {categories.map((cat) => (
              <Link
                ...
                href={
                  cat === "All"
                    ? "/products"
                    : `/products?category=${categoryToSlug(cat)}`
                }
                className={`px-4 py-2 rounded-full text-sm font-medium border transition ${
                  activeCategory === categoryToSlug(cat) ||
          ...
          {filteredProducts.map((product) => {
            ...
            return (
              <Link
                ...
                className="group bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition hover:-translate-y-2 flex flex-col"
              >
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between text-sm font-medium text-accent">
                    <span>{pageContent.productDetailsLabel}</span>
